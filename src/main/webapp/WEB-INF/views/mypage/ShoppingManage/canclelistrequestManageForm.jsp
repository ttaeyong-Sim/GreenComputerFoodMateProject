...
$(document).ready(function(){ //페이지가 준비되면
 ...
 // 반품신청 버튼을 클릭 시
 $(".shipping-status-8").click(function() {
	   	var ordId = $(this).data("ord-id"); // 버튼에서 ord_id 가져오기
	   	if (confirm("해당 상품을 반품신청 하시겠습니까?")) {
	   	    updateOrderStatus(ordId, null, null, 8);
	   	}
 });
 
 // 교환 신청을 클릭시
 $(".shipping-status-9").click(function() {
	   	var ordId = $(this).data("ord-id"); // 버튼에서 ord_id 가져오기
	   	if (confirm("해당 상품을 교환신청 하시겠습니까?")) {
	   	    updateOrderStatus(ordId, null, null, 9);
	   	}
 });
...
<div class="container mt-1">
	<div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
      		<h5 class="mb-0 fw-bold">반품/교환 신청</h5>
    </div>
        ...
        <tbody id="order-list-body">
        	<tr>
        		...
        		<div class="d-flex flex-column gap-1">
				    <!-- 반품 신청 버튼 -->
					<button class="btn btn-outline-secondary btn-sm shipping-status-8" data-ord-id="${order.ord_id}"
					    <c:if test="${not (order.ord_stat eq '1' or order.ord_stat eq '2' or order.ord_stat eq '3')}">disabled</c:if>>
					    반품신청
					</button>
					
					<!-- 교환 신청 버튼 -->
					<button class="btn btn-outline-secondary btn-sm shipping-status-9" data-ord-id="${order.ord_id}"
					    <c:if test="${not (order.ord_stat eq '1' or order.ord_stat eq '2' or order.ord_stat eq '3' )}">disabled</c:if>>
					    교환신청
					</button>
				</div>
